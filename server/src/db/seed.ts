...
const usages: BuildingUsage[] = ['residential', 'commercial', 'industrial', 'public', 'other'];
const stages: LifeCycleStage[] = ['planning', 'construction', 'acceptance', 'registration', 'cancelled'];
...
function generateBuildings(count: number): SeedBuilding[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const floors = usage === 'industrial' ? Math.floor(Math.random() * 5) + 1 :
                   usage === 'commercial' ? Math.floor(Math.random() * 20) + 3 :
...
export function seedData(): void {
  ...
  const parcelBoundary = JSON.stringify({
    type: 'Polygon',
    coordinates: [[
      [CENTER_LNG - GRID_SIZE / 2 - 0.001, CENTER_LAT - GRID_SIZE / 2 - 0.001],
      [CENTER_LNG + GRID_SIZE / 2 + 0.001, CENTER_LAT - GRID_SIZE / 2 - 0.001],
      [CENTER_LNG + GRID_SIZE / 2 + 0.001, CENTER_LAT + GRID_SIZE / 2 + 0.001],
      [CENTER_LNG - GRID_SIZE / 2 - 0.001, CENTER_LAT + GRID_SIZE / 2 + 0.001],
      [CENTER_LNG - GRID_SIZE / 2 - 0.001, CENTER_LAT - GRID_SIZE / 2 - 0.001]
  ...
  const tx = db.transaction(() => {
    for (const b of buildings) {
      ...
      insertBuilding.run(
        b.id, b.name, b.address, b.usage, b.buildYear, b.floors,
        b.undergroundFloors, b.buildingArea, location, outline,
        b.parcelId, b.projectId, b.isCoded, b.currentStage
      );
      
      const stageOrder: LifeCycleStage[] = ['planning', 'construction', 'acceptance', 'registration', 'cancelled'];
      ...
      for (let i = 0; i <= currentIdx; i++) {
        ...
        insertLifecycle.run(
          uuidv4(), b.id, stage, `${year}-0${Math.floor(Math.random() * 9) + 1}-${Math.floor(Math.random() * 28) + 1}`,
          '系统管理员', `${stage === 'planning' ? '规划许可' : stage === 'construction' ? '开工建设' : stage === 'acceptance' ? '竣工验收' : stage === 'registration' ? '产权登记' : '注销登记'}`
        );
      }
    }
  });
