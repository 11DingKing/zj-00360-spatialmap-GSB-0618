...
interface MapContainerProps {
  ...
  onAnalysisResult?: (result: AnalysisResult, polygon: PolygonType) => void;
...
const MapContainer: React.FC<MapContainerProps> = ({
  ...
  const handleAnalyzePolygon = useCallback(async (points: [number, number][]) => {
    if (points.length < 3) return;

    const polygonCoords = [...points, points[0]].map(([lat, lng]) => [lng, lat]);
    const polygon: PolygonType = {
      type: "Polygon",
      coordinates: [polygonCoords as [number, number][]],
    };

    try {
      const response = await redlineApi.analyzeArea(polygon);
      if (response.data.success) {
        onAnalysisResultRef.current?.(response.data.data, polygon);
      }
    } catch (error) {
  ...
  const updatePolygonPreview = useCallback((points: [number, number][], mode: string) => {
    const map = mapRef.current;
    if (!map) return;
    ...
    const color = isAnalyze ? "#F97316" : mode === "polygon" ? "#22C55E" : "#3B82F6";
    const markerColor = isAnalyze ? "#F97316" : "#EF4444";
  ...
  useEffect(() => {
    ...
    map.on("click", (e) => {
      ...
      if (currentDrawMode === "polygon" || currentDrawMode === "analyze") {
  ...
  useEffect(() => {
    if (drawMode === "polygon" || drawMode === "analyze") {
      updatePolygonPreview(polygonPoints, drawMode);
    } else {
      clearTempLayers();
    }
    ...
    <MapContext.Provider value={{ map: mapRef.current }}>
      <div className="relative w-full h-full">
        ...
        <DrawControl
          ...
          onAnalyzeComplete={handleAnalyzePolygon}
        />
      </div>
    </MapContext.Provider>
  );
